...
paradox_generator <- function(n, paradoxality=1, response_t=.25, stratum_t=.4, equal_variance=T) {
  ev <- ifelse(equal_variance, 0, 1)
  response <- rbinom(n, 1, response_t)
  stratum <- rbinom(n, 1, stratum_t)

  shift <- 2*(1-paradoxality)

  mus <- ifelse(response == stratum, 0, 1) + ifelse(response==1, shift, 0)
